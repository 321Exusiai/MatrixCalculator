...
在 `vector.h` 中，我们实现了基于 $L^2$ 范数的归一化：
$$
...
* **移动语义 (Move Semantics)**：在 `Matrix` 类中广泛使用 `std::move`，将复杂度从 $O(n^2)$ 的拷贝降至 $O(1)$ 的指针转移。
* **数值稳定性**：在消元时搜索当前列绝对值最大的元素作为主元，抑制浮点误差：

```cpp
int max_row = find_max_pivot(current_col);
...
> **Developer:** [321Exusiai](https://www.google.com/search?q=https://github.com/321Exusiai)
> **Course:** 线性代数与 C++ 编程实践 (Freshman Year @ 2024)
