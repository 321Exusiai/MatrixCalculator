...
```
🚀 性能优化特性移动语义 (Move Semantics)：在 Matrix 类中广泛使用 std::move，将复杂度从 $O(n^2)$ 的拷贝降至 $O(1)$ 的指针转移。数值稳定性：在消元时搜索当前列绝对值最大的元素作为主元，抑制浮点误差。核心逻辑如下：C++int max_row = find_max_pivot(current_col);
swap_rows(current_row, max_row);
```
```
鲁棒性 (Robustness)：利用 std::invalid_argument 对维度不匹配或不可逆矩阵进行严格校验。
```
🛠 快速上手1. 编译 (Requires C++17)Bashg++ -std=c++17 main.cpp -o matrix_calc
2. 运行Bash./matrix_calc
```
Developer: 321Exusiai Course: 高等代数 (WHU Freshman Year @ 2025)
